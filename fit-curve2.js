 ...
 */
function fitCurve(points, maxError, maxNum, ts, progressCallback) {
    ...
    return fitCubic(points, leftTangent, rightTangent, maxError, maxNum, 0, ts, progressCallback);
}
 ...
 * @returns {Array<Array<Array<Number>>>} Array of Bezier curves, where each element is [first-point, control-point-1, control-point-2, second-point] and points are [x, y]
 */
function fitCubic(points, leftTangent, rightTangent, error, maxNum, recursionDepth = 0, ts, progressCallback) {
    ...
    u = ts ? ts : chordLengthParameterize(points);
    [bezCurve, maxError, splitPoint] = generateAndReport(points, u, u, leftTangent, rightTangent, progressCallback)
    // console.log('maxError', maxError, 'error', error, 'maxNum');
    ...
    // if (error > 1 && maxError < error*error || error <= 1 && maxError > Math.sqrt(error)) {
        ...
        for (i = 0; i < MaxIterations; i++) {
            uPrime = ts ? ts : reparameterize(bezCurve, points, uPrime);
            [bezCurve, maxError, splitPoint] = generateAndReport(points, u, uPrime, leftTangent, rightTangent, progressCallback);
            // console.log(i, maxError/prevErr, splitPoint === prevSplit);
    ...
    */
    beziers = beziers.concat(fitCubic(points.slice(0, splitPoint + 1), leftTangent, toCenterTangent,    error, maxNum, recursionDepth + 1, ts?.slice(0, splitPoint + 1), progressCallback));
    beziers = beziers.concat(fitCubic(points.slice(splitPoint),        fromCenterTangent, rightTangent, error, maxNum, recursionDepth + 1, ts?.slice(splitPoint), progressCallback));
    return beziers;
};
