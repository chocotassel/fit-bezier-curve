 ...
 */
function fitCurve(points, maxError, maxNum, progressCallback) {
    ...
    return fitCubic(points, leftTangent, rightTangent, maxError, maxNum, 0, progressCallback);
}
 ...
 * @returns {Array<Array<Array<Number>>>} Array of Bezier curves, where each element is [first-point, control-point-1, control-point-2, second-point] and points are [x, y]
 */
function fitCubic(points, leftTangent, rightTangent, error, maxNum, recursionDepth = 0, progressCallback) {
    ...
    */
    beziers = beziers.concat(fitCubic(points.slice(0, splitPoint + 1), leftTangent, toCenterTangent,    error, maxNum, recursionDepth + 1, progressCallback));
    beziers = beziers.concat(fitCubic(points.slice(splitPoint),        fromCenterTangent, rightTangent, error, maxNum, recursionDepth + 1, progressCallback));
    return beziers;
};
...
function computeMaxError(points, bez, parameters) {
    // if (points.length === 2) {
    //     var dist, maxDist = 0, splitPoint = Math.floor(points.length / 2), point, t;

    //     for (let i = 0; i < points.length; i++) {
    //         point = points[i];
    //         t = point[0]; // Assuming t is directly based on x-coordinate and is normalized

    //         // Assuming bezier.q calculates the point on the Bezier curve for given t
    //         let curvePoint = bezier.q(bez, t);
    //         dist = maths.vectorLen(maths.subtract(curvePoint, point)); // Assuming this function computes Euclidean distance
