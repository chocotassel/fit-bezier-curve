 ...
 */
function fitCurve(type, points, maxError, maxNum, ts, progressCallback) {
    ...
    return fitCubic(type, points, leftTangent, rightTangent, maxError, maxNum, ts, 0, progressCallback);
}
 ...
 * @returns {Array<Array<Array<Number>>>} Array of Bezier curves, where each element is [first-point, control-point-1, control-point-2, second-point] and points are [x, y]
 */
function fitCubic(type, points, leftTangent, rightTangent, error, maxNum, ts, recursionDepth = 0, progressCallback) {
    const MaxIterations = 200;   //Max times to try iterating (to find an acceptable curve)
    const MaxRecursionDepth = Math.log2(maxNum)
    ...
    // if (error > 1 && maxError < error*error || error <= 1 && maxError > Math.sqrt(error)) {
        // uPrime = u;
        // prevErr = maxError;
        ...
        //     console.log('Iterations', i, u, uPrime);
        ...
        //         if((errChange > .99) && (errChange < 1.01)) {
        //             break;
        //         }
        //     }
    ...
    if (recursionDepth >= MaxRecursionDepth) {
        return [bezCurve];  // Return the current best effort if recursion limit is reached
    }
    ...
    // console.log('splitPoint', splitPoint, points.length);
    ...
    */
    beziers = beziers.concat(fitCubic(type, points.slice(0, splitPoint + 1), leftTangent, toCenterTangent,    error, maxNum, ts, recursionDepth + 1, progressCallback));
    beziers = beziers.concat(fitCubic(type, points.slice(splitPoint),        fromCenterTangent, rightTangent, error, maxNum, ts, recursionDepth + 1, progressCallback));
    return beziers;
};

function generateAndReport(points, paramsOrig, paramsPrime, leftTangent, rightTangent, ts, progressCallback) {
    var bezCurve, maxError, splitPoint;

    bezCurve = generateBezier(points, paramsPrime, leftTangent, rightTangent, ts, progressCallback);
 ...
 */
function generateBezier(points, parameters, leftTangent, rightTangent, ts) {
    // debugger
 ...
 */
function chordLengthParameterize(points, type) {
    // var u = [], currU, prevU, prevP;
    var u = []
    if (type === 1) {
      if (points[0].length === 2) {
        const prevP = points[0]
        const r = points[points.length - 1][0] - points[0][0]
        points.forEach((p, i) => {
          const x = (p[0] - prevP[0]) / r
          u.push(x)
        })
        ...
        let prevP = points[0]
        points.forEach((p, i) => {
          length += maths.vectorLen(maths.subtract(p, prevP))
          u.push(length)
          prevP = p
        })
        u = u.map(x => x / length)
...
function computeMaxError(points, bez, parameters) {
    ...
    for (i = 0, count = points.length; i < count; i++) {
        ...
        // draw(t, point[1], 'blue', 1)
        // t = (point[0] - points[0][0]) / (pointås[points.length - 1][0] - points[0][0]);
        // t = parameters[i];
        // console.log(point[0], t);
        // draw(point[0]+0.01, t, 'red', 1)

        v = maths.subtract(bezier.q(bez, t), point);
        dist = maths.sum(maths.squareItems(v));
